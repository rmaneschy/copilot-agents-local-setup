...
| **make** | Via Git Bash (incluso no Git for Windows) | Pré-instalado |
| **jq** (opcional) | `winget install jqlang.jq` | `apt install jq` ou já disponível |

---

## Makefile — Orquestrador Unificado

O **Makefile** é o ponto de entrada único para todas as operações. Ele detecta automaticamente o sistema operacional e executa o script correto (`.ps1` no Windows, `.sh` no Linux).
...
make install-code-search       # codebase-memory-mcp
make install-code-navigation   # Serena MCP
make install-observability     # Arize Phoenix
...
|:---:|:---|:---:|:---|
| 1 | `make install-code-search` | ~2 min | Baixa binário estático (~15MB), instala no PATH do usuário, configura `mcp.json` |
| 2 | `make install-code-navigation` | ~3 min | Instala `uv` + Serena MCP (LSP server) sem admin |
| 3 | `make install-observability` | ~3 min | Instala Arize Phoenix (observabilidade) e inicia o servidor local |
| 4 | `make index` | ~5 min* | Descobre repos via `.git` e indexa o knowledge graph (*varia com tamanho do workspace) |
...
```bash
make install && make index
```

**Equivalente sem Makefile (execução direta):**
...
| "Server not found" | Executar `make check` para verificar binários no PATH |
| "Connection refused" | **Windows:** `where.exe codebase-memory-mcp` / **Linux:** `which codebase-memory-mcp` |
| Tools aparecem mas não retornam resultados | Executar `make index` para indexar o workspace |
| Phoenix não recebe traces | Verificar se o `mcp.json` usa a versão com `--phoenix` e se Phoenix está rodando (`make status-phoenix`) |
...
```bash
# Via Makefile (detecta OS automaticamente)
make upgrade
```
...
|:---|:---|:---|
| **Via Makefile** | `make mode-offline` | `make mode-online` |
...
```text
Makefile                                 # Orquestrador unificado (detecta OS, chama .ps1 ou .sh)
